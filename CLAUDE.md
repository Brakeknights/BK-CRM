...
2. `git branch --show-current` — confirm you are on `claude/busy-ritchie-GuJOH` (or the current feature branch); if not, switch: `git checkout claude/busy-ritchie-GuJOH`
...
- Current feature branch: `claude/busy-ritchie-GuJOH` (Phase 7A refinements round 2 — merged to dev)
