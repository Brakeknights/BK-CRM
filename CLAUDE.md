...
- [ ] 🔒 **Upgrade nodemailer 8 → 9** (flagged 2026-06-19 by npm audit, 1 of 3 high-severity items). Low real-world risk for us: all three CVEs hit features we don't use (`List-*` headers, `jsonTransport`, `raw` message option) — we send via plain SMTP. But it's the only flagged package that's both deployed and written to by our code, so bump it for hygiene. **It's a major-version jump — do it on its own branch and verify a test email still sends on dev before going to master.** (The other two audit items — `hono` and `form-data` — need no action: `hono` is local-only via the Hostinger MCP and never deployed; `form-data` is internal to the Square SDK with no attacker-controlled input. **REMIND THE OWNER about the nodemailer upgrade at the start of a future session.**)
