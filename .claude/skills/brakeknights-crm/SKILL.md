...
| 2026-06-27 batch (PR #64) | **Inline-editable customer card** (single source of truth; lead page + profile share it; tap to edit phone/email/address/name/vehicle, saves via AJAX `POST /admin/customer/:id/field` [allowlisted cols] + `/vehicle/:vid/update` + `/vehicle/add`; fixed "None on file"). **Data unison** (`customers.syncCustomerData`). **Appointment auto-attach** to an open lead. **Short pay** ("Amount Received" → `receipts.billed_total`; receipt shows Subtotal+Adjustment+Total Paid). **Tips** (`receipts.tip`, separate from sales, Revenue "Tips Collected"; payment = Cash/Zelle/Credit-Debit/Other). **Shop supplies per-service** (adds on select, subtracts on remove, never re-adds on draft rebuild). **1-week check-in from every receipt + CC owner.** **Maps autocomplete loaded once in shell** (`data-addr-ac` + `window.bkInitAddrAC()`). Floating Save button on main forms; Quick Quote draft no longer drops services (`qRestoring` guard). Reminder emails get Google + Apple/Outlook calendar buttons. |

**Current status (2026-06-27): Phases 1–7C + 8E/8F + the 2026-06-24 and 2026-06-27 batches are LIVE on master (tip `cd1a67d`, dev and master in sync). Phase 8 automated quotes still pending the per-vehicle pricing matrix. Auto-deploy is broken — deploy via the Hostinger MCP archive (CLAUDE.md DEPLOYMENT BROKEN note).**

**Schema additions since this skill was first written (also in `db.js`):** tables `customers`, `customer_vehicles`, `customer_addresses`, `push_subscriptions`, `personal_events`, `quick_drafts`, plus pricing/vehicle-mapping foundation tables. New columns: `leads.customer_id`/`vin`/`internal_notes`/`interaction_notes`/`archived`, `receipts.custom_line_items`/`filed_at`/`billed_total` (billed amount when a short/over pay is recorded; `total` holds what was actually collected)/`tip` (gratuity, not taxed, not in sales), `followups.kind` (`'advisory'` default | `'review_checkin'`).

**Inline customer editing (2026-06-27):** the unified customer card edits fields in place. Backend: `POST /admin/customer/:id/field` updates ONE column from a fixed allowlist (`first_name,last_name,phone,email,home_address`) — never trust a column name from the request. Vehicles: `/vehicle/:vid/update` + `/vehicle/add`. Client: rows tagged `.bk-ief` (contact) / `.bk-veh` (vehicles); the event-delegated handler lives in the admin shell (survives bkBoost swaps). Helpers `inlineField()` / `inlineVehicleList()` in `routes/admin.js`.

**Shop supplies behavior (important):** it is per selected service ($10 each), handled in the shared pricing block JS (`quotePricingJs`): `AddPriceRow(svc, addSupplies)` only adds when `addSupplies` is true (user ticks a service); `RemovePriceRow` subtracts; rebuilds from a saved/auto-saved draft call neither flag so they NEVER re-add. Do not reintroduce per-rebuild addition.
